...
BEGIN
  ...
  -- Create admin perfil for the new company
  IF new_empresa_id IS NOT NULL THEN
    INSERT INTO public.perfis (nome, ver_todas_conversas, idempresa)
    VALUES ('Administrador', true, new_empresa_id)
    RETURNING id INTO new_perfil_id;
  END IF;

  INSERT INTO public.profiles (user_id, nome, email, telefone, empresa_id, perfil_id)
  VALUES (
    NEW.id,
